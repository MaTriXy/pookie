...
- Verify `OPENAI_API_KEY` is set in `.env` (or `.env.local`)
- Confirm the key starts with `sk-` and is not expired or revoked at platform.openai.com/api-keys
- Restart the dev server after editing `.env` — Next.js doesn't pick up env-var changes hot
