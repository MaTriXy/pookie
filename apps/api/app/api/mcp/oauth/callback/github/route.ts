import { finishGitHubOAuth } from "@/server/mcp/github-oauth";
...
export const GET = async (request: Request): Promise<Response> => {
  ...
  try {
    const { userId, serverName, channelId, teamId } = await finishGitHubOAuth({
      code,
      state,
    });
    ...
    console.error("[mcp-github-oauth-callback]", error);
...
<head>
  ...
  <title>GitHub ${isSuccess ? "Connected" : "Error"}</title>
  <style>
