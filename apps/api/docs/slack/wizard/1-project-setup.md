...
3. **Generate a specific plan** using the template structure:
   ...
   - Event Handlers (`onNewMention`, `onSubscribedMessage`, `onSlashCommand`, `onAction`, `onReaction`)
...
### Step 1.3: Confirm LLM Use (if using AI)
...
>
> 1. **Yes** — uses OpenAI via the AI SDK (`@ai-sdk/openai`). Requires `OPENAI_API_KEY` in Phase 3.
> 2. **No LLM needed** — simple automation bot (slash commands, integrations, etc.). Can add AI later.

Based on their choice:

**If Yes (default):**

- The project will include `ai` and `@ai-sdk/openai` packages
- Store this choice for Phase 3 — they'll need `OPENAI_API_KEY` in `.env`
