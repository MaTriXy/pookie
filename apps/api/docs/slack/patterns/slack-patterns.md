...
await thread.post(
  <Card title="Hello!">
    ...
    <Actions>
      <Button id="button_click" value="button_value" style="primary">
        Click Me
      </Button>
    </Actions>
  </Card>,
...
await thread.post(
  <Card>
    ...
    <Actions>
      <Button id="delete_item" value={itemId} style="danger">
        Delete
      </Button>
...
bot.onNewMention(async (thread, message) => {
  await thread.subscribe();
  await thread.post(
    "I'm listening! Send me follow-up messages in this thread.",
  );
});
