...
This document provides common patterns and archetypes for Slack agents built with the Chat SDK. Use this as a reference when generating custom implementation plans based on user requirements.
...
- [ ] `@chat-adapter/state-redis` for thread-level persistence
...
JSX components (`<Card>`, `<Button>`, `<Modal>`)
